...
%% 检查连接是否超时仍为有数据通信
handle_call(heart_beat, _From, _State = #state{socket = Socket,
                                               recv_cnt = ReceiveBags, keep_still_times = LostTimes}) ->
    NowConnBags = inet:getstat(Socket, [recv_cnt]),
    NewState =
        case ReceiveBags == NowConnBags of
            true ->  #state{socket = Socket, keep_still_times = LostTimes + 1};
            false -> #state{socket = Socket, recv_cnt = NowConnBags, keep_still_times = 0}
        end,
    case LostTimes >= 10 of
